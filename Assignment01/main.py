...
def findSnarePosition(snareFilename, drumloopFilename):
    x=readfile(snareFilename)
    y=readfile(drumloopFilename)
    lenx=len(x)
    pos1= {}
    pos2=[]
    r=crossCorr(x, y)
...
# Q1
    x=readfile('./Assignment01/download.wav')
    lenx = len(x)
    print('x',x,lenx)
    y=readfile('./Assignment01/drum_loop.wav')
    leny = len(y)
    print('y',y,leny)
    Z=crossCorr(x,y)
    plt.plot(Z)
    ...
    plt.show()
#Q2
    pos,pos1=findSnarePosition('./Assignment01/download.wav', './Assignment01/drum_loop.wav')
